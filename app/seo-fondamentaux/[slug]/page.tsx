...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    ...
    alternates: {
      canonical: `https://www.seotika.com/seo-fondamentaux/${article.slug}`,
    },
  };
}

export default function SeoFondamentauxArticlePage({ params }: PageProps) {
  const article = clusterData.childPages.find((p) => p.slug === params.slug);
  ...
  const jsonLd = generateArticleJsonLd(article, 'seo-fondamentaux', author?.name || 'SEOtika');
  ...
  return (
    <>
      ...
      <div className="container-custom py-8">
        <Breadcrumb
          items={[
            { label: 'Fondamentaux SEO', href: '/seo-fondamentaux' },
            { label: article.title, href: `/seo-fondamentaux/${article.slug}` },
          ]}
        />
        ...
        <div className="grid lg:grid-cols-[1fr_280px] gap-8">
          <article>
            ...
            <RelatedArticles articles={relatedArticles} clusterSlug="seo-fondamentaux" />

            <nav className="flex items-center justify-between mt-12 pt-8 border-t">
              {prev ? (
                <Link
                  href={`/seo-fondamentaux/${prev.slug === 'index' ? '' : prev.slug}`}
              ...
              {next ? (
                <Link
                  href={`/seo-fondamentaux/${next.slug}`}
                  className="flex items-center gap-2 text-muted-foreground hover:text-foreground transition-colors"
                >
                  <div>
          ...
          <aside className="hidden lg:block">
            <div className="sticky-toc space-y-6">
              ...
              <div className="border rounded-lg p-4">
                <h3 className="font-semibold mb-3">Module Fondamentaux</h3>
                <Button asChild variant="outline" size="sm" className="w-full">
                  <Link href="/seo-fondamentaux">Voir tous les articles</Link>
                </Button>
              </div>
            </div>
          </aside>
